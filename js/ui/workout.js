...
import { el, videoBlock, fmtTime, toast, sheet } from './common.js';
import { get, S, update, todayISO } from '../state.js';
import { buildWorkout, recordProgress, detectPRs, warmupFor, alternativesFor, suggestWeight, nextWeight, stepDown } from '../engine.js';
...
export function openWorkout(session, adjust, ctx) {
  const app = document.getElementById('app');
  keepAwake();

  const startedAt = Date.now();
  const slots = buildWorkout(session, adjust);
  const iso = todayISO();
  ...
  function stopRest() { clearInterval(restIv); restEl?.remove(); restEl = null; }
  ...
  function renderAll() {
    ...
    app.append(el('div', { class: 'player-head spread' },
      el('button', { class: 'btn-sm btn-ghost', onclick: () => { if (confirm('Workout afbreken? Ingevoerde sets gaan verloren.')) { stopRest(); releaseWake(); ctx.render(); } } }, '‹ Terug'),
  ...
  function slotCard(slot, si) {
    ...
    card.append(el('div', { class: 'tiny dim mt' },
    ...
    card.append(el('div', { class: 'tiny mt', style: 'color:var(--primary)' }, '↳ ' + slot.suggestion.text));
    if (slot.note) card.append(el('p', { class: 'tiny dim mt mb0' }, slot.note));

    // acties: video/uitleg + wissel
    const detail = el('div', { style: 'display:none' });
    const toggle = el('button', { class: 'btn-sm btn-ghost mt', onclick: () => {
      const open = detail.style.display !== 'none';
      detail.style.display = open ? 'none' : 'block';
      toggle.textContent = open ? '▸ Video & uitleg' : '▾ Verberg uitleg';
      if (!open && !detail.dataset.filled) {
        detail.dataset.filled = '1';
        if (ex) { detail.append(videoBlock(ex)); detail.append(el('p', { class: 'tiny dim' }, ex.instructions)); if (ex.tips) detail.append(el('p', { class: 'tiny', style: 'color:var(--primary)' }, '💡 ' + ex.tips)); }
      }
    } }, '▸ Video & uitleg');
    const swapBtn = el('button', { class: 'btn-sm btn-ghost mt', onclick: () => openSwap(si) }, '⇄ Wissel oefening');
    card.append(el('div', { class: 'row' }, toggle, swapBtn), detail);
    ...
    card.append(el('div', { class: 'mt tiny dim', style: 'display:grid;grid-template-columns:44px 1fr 1fr 54px 48px;gap:8px;text-align:center' },
  ...
  // RIR 0 of reps onder de range → resterende sets lichter (en bij herhaling: laatste set optioneel).
    ...
    if ((missed || ground) && remaining.length) {
      ...
      if (isWeighted) {
        ...
        if (nw > s.weight) {
          ...
          showHint(`RIR ${s.rir} op de rep-top — volgende set naar ${nw} kg.`);
